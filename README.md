...
### Декларации: строгий мультимодальный маршрут

Для маршрута от выбранной декларации теперь используется фиксированная логика:
...
- декларационный маршрут **больше не откатывается самопроизвольно в `road_only`**, если существует нужный мультимодальный сценарий;
- для деклараций frontend отправляет запрос как **`multimodal`**;
- backend собирает именно цепочку **`road_start → rail → road_end`**;
- если полная цепочка не собрана, API возвращает **`404 multimodal route not found`**.

### Диагностика мультимодального маршрута

Если маршрут не найден, backend теперь может вернуть расширенную диагностику, чтобы сразу понять, на каком плече возникла проблема:
...
Это сильно упрощает отладку случаев, когда ж/д путь между станциями существует, но полный маршрут от точки до точки не строится.
...
### 2. Серверный режим через nginx и `/railmap`
...
В актуальной версии маршрут строится строго по цепочке:
...
- если полный сценарий не собран, API возвращает `404 multimodal route not found`.
...
- маршрут будет собран по цепочке **авто → ж/д → авто**;
